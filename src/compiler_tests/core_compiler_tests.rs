...
mod place_system_tests {
    ...
    fn test_local_place_creation() {
        ...
        assert_eq!(
            place.load_instruction_count(),
            1,
            "Local load should be 1 instruction"
        );
        assert_eq!(
            place.store_instruction_count(),
            1,
            "Local store should be 1 instruction"
        );
    }
    ...
    fn test_memory_place_creation() {
        ...
        assert!(
            place.requires_memory_access(),
            "Memory place should require memory access"
        );
        assert_eq!(
            place.load_instruction_count(),
            2,
            "Memory load should be 2 instructions"
        );
        assert_eq!(
            place.store_instruction_count(),
            2,
            "Memory store should be 2 instructions"
        );
    }
    ...
    fn test_field_projection() {
        ...
        assert_eq!(
            field.load_instruction_count(),
            3,
            "Field projection should be 3 instructions"
        );
        assert!(
            field.load_instruction_count() <= 5,
            "Field projection should be WASM-efficient"
        );
    }
