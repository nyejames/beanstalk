...
use crate::compiler_frontend::compiler_messages::{
    CompileTimeEvaluationErrorReason, DiagnosticPayload, InvalidDeclarationReason,
};
    ...
    for (index, node) in ast.nodes.iter().enumerate() {
        match &node.kind {
                ...
                if path.name_str(&project.frontend.string_table) == Some("Outer") =>
            {
                ...
                outer_field_type = Some(fields[0].value.diagnostic_type.clone());
            }
...
fn reports_circular_imports_through_frontend_header_sorting() {
    ...
    let string_table = &project.frontend.string_table;
    assert!(
        bag.diagnostics().iter().any(|diagnostic| {
            ...
            path.contains("AStruct") || path.contains("BStruct")
        }),
        "expected circular dependency diagnostic, got: {bag:?}"
    );
}
        ...
        hir.blocks
            ...
            .any(|statement| match &statement.kind {
                HirStatementKind::Call {
                    ...
                    id.name() == "__bs_collection_set"
                }
                _ => false,
            }),
        "expected collection set(...) to lower into a host call target"
    );
...
fn same_file_forward_constant_reference_rejected() {
    ...
    let entry_file_id = project
        .frontend
        .source_files
        .get_by_canonical_path(&project.entry_file)
        .map(|identity| identity.file_id);

    let options = HeaderParseOptions {
        entry_file_id,
        project_path_resolver: project.frontend.project_path_resolver.clone(),
    };
    ...
    for file_tokens in tokenized_files {
        let output = prepare_file_from_tokens(
            ...
            &project.entry_file,
            &options,
            &project.frontend.external_package_registry,
            &mut project.frontend.string_table,
            0,
            0,
        )
        .expect("per-file preparation should succeed");
        prepared_outputs.push(output);
    }
    ...
    assert!(
        bag.diagnostics().iter().any(|diagnostic| matches!(
            &diagnostic.payload,
            DiagnosticPayload::CompileTimeEvaluationError {
                reason: CompileTimeEvaluationErrorReason::SameFileForwardConstantReference,
                ..
            } if diagnostic.kind.descriptor().code == "BST-RULE-0053"
        )),
        "expected same-file forward-reference diagnostic, got: {bag:?}",
    );
}
...
fn choice_definitions_are_collected_once() {
    ...
    let status_type_id = ast
        .type_environment
        .nominal_id_for_path(&status_definitions[0].nominal_path)
        .and_then(|nominal_id| ast.type_environment.type_id_for_nominal_id(nominal_id))
        .expect("Status should be registered in the TypeEnvironment");
    let variants = ast
        .type_environment
        .variants_for(status_type_id)
        .expect("Status should expose variants through the TypeEnvironment");
...
fn struct_default_errors_on_visible_non_constant() {
    ...
    assert!(
        messages.diagnostics.iter().any(|diagnostic| matches!(
            diagnostic.payload,
...
fn rejects_virtual_package_import_of_missing_symbol() {
    ...
    assert!(
        ...
        "expected missing package symbol diagnostic, got: {bag:?}",
    );
}
    ...
    assert!(
        messages.error_diagnostics().any(|diagnostic| matches!(
            ...
            DiagnosticPayload::InvalidDeclaration {
                reason: InvalidDeclarationReason::ExternalTypeLiteralConstruction,
                ..
            }
        )),
...
fn external_type_resolves_in_type_annotation() {
    ...
    assert!(
        !messages
            .error_diagnostics()
            .any(|diagnostic| matches!(&diagnostic.payload, DiagnosticPayload::UnknownName { .. })),
        "'IO' should resolve as a known external type, not produce an unknown-name diagnostic. Got: {:?}",
        messages
            .error_diagnostics()
            .map(|diagnostic| &diagnostic.payload)
            .collect::<Vec<_>>()
