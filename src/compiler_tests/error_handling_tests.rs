...
mod error_handling_tests {
    ...
    fn test_error_types() {
        ...
        // Test syntax error
        let syntax_error =
            CompileError::new_syntax_error("Test syntax error".to_string(), location.clone());
        assert_eq!(
        ...
        assert!(
            syntax_error.msg.contains("Test syntax error"),
            "Should contain error message"
        );

        // Test rule error
        let rule_error =
            CompileError::new_rule_error("Test rule error".to_string(), location.clone());
        assert_eq!(
            rule_error.error_type,
            ErrorType::Rule,
            "Should be rule error"
        );
        assert!(
            rule_error.msg.contains("Test rule error"),
            "Should contain error message"
        );

        // Test type error
        let type_error =
            CompileError::new_type_error("Test type error".to_string(), location.clone());
        assert_eq!(
        ...
        assert!(
            type_error.msg.contains("Test type error"),
            "Should contain error message"
        );
    }
    ...
    fn test_compiler_error_handling() {
        ...
        assert!(
            error.msg.contains("Test unimplemented feature"),
    ...
    fn test_file_error_handling() {
        ...
        assert!(
            error.msg.contains("File not found"),
            "Should contain error message"
        );
    ...
    fn test_error_message_quality() {
        ...
        assert!(
            !error.msg.contains("panic"),
            "Error message should not mention panic"
        );
        assert!(
            !error.msg.contains("unwrap"),
            "Error message should not mention unwrap"
        );
    }
    ...
    fn test_error_macro_patterns() {
        ...
        let syntax_error =
            CompileError::new_syntax_error("Test syntax error with location".to_string(), location);
    ...
    fn test_descriptive_error_messages() {
        ...
        assert!(
            descriptive_error.msg.len() > 50,
            "Error message should be descriptive"
        );
        assert!(
            descriptive_error.msg.contains("test_var"),
            "Should mention specific variable"
        );
        assert!(
            descriptive_error.msg.contains("Shadowing is not supported"),
            "Should explain the rule"
        );
        assert!(
            descriptive_error.msg.contains("Try using"),
        ...
        assert!(
            compiler_error.msg.contains("my_var"),
            "Should mention specific variable"
        );
        assert!(
            compiler_error.msg.contains("line 42"),
            "Should include location"
        );
        assert!(
            compiler_error.msg.contains("coming soon"),
            "Should provide context"
        );
        assert!(
            compiler_error.msg.contains("try using"),
    ...
    fn test_error_message_formatting() {
        ...
        let rule_error =
            CompileError::new_rule_error("Test rule violation".to_string(), location.clone());
        let type_error =
            CompileError::new_type_error("Test type mismatch".to_string(), location.clone());
        let syntax_error =
            CompileError::new_syntax_error("Test syntax issue".to_string(), location);

        // All error messages should be properly formatted
        assert!(
            !rule_error.msg.is_empty(),
            "Rule error message should not be empty"
        );
        assert!(
            !type_error.msg.is_empty(),
            "Type error message should not be empty"
        );
        assert!(
            !syntax_error.msg.is_empty(),
            "Syntax error message should not be empty"
        );

        // Error messages should be helpful
        assert!(
            rule_error.msg.len() > 10,
            "Rule error should be descriptive"
        );
        assert!(
            type_error.msg.len() > 10,
            "Type error should be descriptive"
        );
        assert!(
            syntax_error.msg.len() > 10,
            "Syntax error should be descriptive"
        );
    }
    ...
    fn test_source_location_preservation() {
        ...
        let error =
            CompileError::new_rule_error("Test error with location".to_string(), location.clone());

        // Location should be preserved
        assert_eq!(
            error.location.start_pos.line_number, 42,
            "Line number should be preserved"
        );
        assert_eq!(
            error.location.start_pos.char_column, 15,
            "Column number should be preserved"
        );
    }
    ...
    fn test_error_handling_robustness() {
        ...
        assert_eq!(
            long_error.error_type,
            ErrorType::Syntax,
            "Should handle long messages"
        );
        assert!(
            long_error.msg.contains(&long_message),
            "Should preserve long messages"
        );
    }
    ...
    fn test_mir_error_message_quality() {
        ...
        assert!(
            undefined_var_error.msg.contains("my_var"),
            "Should mention specific variable"
        );
        assert!(
            undefined_var_error.msg.contains("declared before mutation"),
            "Should explain the rule"
        );
        assert!(
            undefined_var_error.msg.contains("Did you mean"),
            "Should provide suggestions"
        );
        ...
        assert_eq!(type_error.error_type, ErrorType::Type);
        assert!(
            type_error.msg.contains("must be boolean"),
            "Should explain type requirement"
        );
        assert!(
            type_error.msg.contains("found Int"),
            "Should mention actual type"
        );
        assert!(
            type_error.msg.contains("Try using"),
            "Should provide suggestions"
        );
        ...
        assert_eq!(compiler_error.error_type, ErrorType::Compiler);
        assert!(
            compiler_error.msg.contains("not yet implemented"),
            "Should indicate unimplemented feature"
        );
        assert!(
            compiler_error.msg.contains("line 42"),
            "Should include location"
        ...
        assert!(
            compiler_error.msg.contains("Try breaking down"),
            "Should provide workaround"
        );
    }
    ...
    fn test_mir_error_consistency() {
        ...
        assert!(
            rule_error.msg.contains("my_func"),
            "Rule errors should mention specific names"
        );
        assert!(
            rule_error.msg.contains("must be"),
            "Rule errors should explain requirements"
        );
        assert!(
            rule_error.msg.contains("Make sure"),
            "Rule errors should provide guidance"
        );
        ...
        assert!(
            type_error.msg.contains("String and Int"),
            "Type errors should mention specific types"
        );
        assert!(
            type_error.msg.contains("must be"),
            "Type errors should explain requirements"
        );
        ...
        assert!(
            compiler_error.msg.contains("not yet implemented"),
            "Compiler errors should indicate missing features"
        );
        assert!(
            compiler_error.msg.contains("line 10"),
            "Compiler errors should include location when available"
        );
        assert!(
            compiler_error.msg.contains("needs to be added"),
            "Compiler errors should explain what's needed"
        );
    }
    ...
    fn test_borrow_checker_error_integration() {
        ...
        assert!(
            borrow_error.msg.contains("Cannot borrow"),
            "Should explain borrow violation"
        );
        assert!(
            borrow_error.msg.contains("Consider using"),
            "Should provide suggestions"
        );
        ...
        assert!(
            move_error.msg.contains("moved value"),
            "Should explain move violation"
        );
        assert!(
            move_error.msg.contains("Try using references"),
    ...
    fn test_error_message_validation() {
        ...
        assert!(
            issues.is_empty(),
            "Good error should have no validation issues"
        );
        ...
        assert!(
            !issues.is_empty(),
            "Bad error should have validation issues"
        );
        ...
        assert!(
            issues
                .iter()
                .any(|issue| issue.contains("internal implementation details"))
        );
        ...
        assert!(
            issues
                .iter()
                .any(|issue| issue.contains("should include suggestions"))
        );
    }
