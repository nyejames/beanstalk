...
fn test_basic_type_definitions() {
    ...
    validator
        ...
        .expect("Type definitions should be valid");
}
...
fn test_type_safe_control_flow() {
    ...
    validator
        .validate_all(&wasm_bytes)
        .expect("Type-safe control flow should be valid");
}
...
fn test_local_variable_types() {
    ...
    types
        .ty()
        .function(vec![ValType::I32, ValType::F32], vec![ValType::F64]);
    module.section(&types);
    ...
    validator
        .validate_all(&wasm_bytes)
        .expect("Typed locals should be valid");
}
...
fn test_type_conversions() {
    ...
    validator
        .validate_all(&wasm_bytes)
        .expect("Type conversions should be valid");
}
...
fn test_multi_value_returns() {
    ...
    validator
        .validate_all(&wasm_bytes)
        .expect("Multi-value returns should be valid");
}
...
fn test_type_mismatch_validation() {
    ...
    func.instruction(&Instruction::F32Const(wasm_encoder::Ieee32::new(
        PI.to_bits(),
    )));
    func.instruction(&Instruction::End);
...
fn test_stack_type_consistency() {
    ...
    validator
        .validate_all(&wasm_bytes)
        .expect("Consistent stack types should be valid");
}
...
fn test_block_type_consistency() {
    ...
    validator
        .validate_all(&wasm_bytes)
        .expect("Block type consistency should be valid");
}
...
fn test_invalid_local_index() {
    ...
    func.instruction(&Instruction::LocalGet(1)); // This should cause a validation error
    ...
    assert!(
        result.is_err(),
        "Invalid local index should cause validation error"
    );
}
