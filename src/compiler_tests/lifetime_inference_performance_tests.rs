...
fn benchmark_single_size(size: usize) -> PerformanceMetrics {
    ...
    let result = dataflow.analyze_to_fixpoint().expect("Dataflow should converge");
...
fn create_synthetic_cfg(size: usize) -> ControlFlowGraph {
    ...
    for i in 0..size {
        ...
        cfg.add_node(node_id, crate::compiler::borrow_checker::types::CfgNodeType::Statement);
...
fn validate_linear_scaling(results: &[(usize, PerformanceMetrics)]) {
    ...
    let complexity_factors: Vec<f64> = results.iter()
        .map(|(_, metrics)| metrics.complexity_factor)
        .collect();

    let min_factor = complexity_factors.iter().fold(f64::INFINITY, |a, &b| a.min(b));
    let max_factor = complexity_factors.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    
    let factor_ratio = max_factor / min_factor;
    
    println!("Complexity factor range: {:.2} - {:.2}", min_factor, max_factor);
    ...
    if factor_ratio <= MAX_ACCEPTABLE_RATIO {
        ...
        println!("  Factor ratio {} exceeds maximum acceptable ratio {}", 
                 factor_ratio, MAX_ACCEPTABLE_RATIO);
    }

    // Check that time per node doesn't grow exponentially
    let times_per_node: Vec<Duration> = results.iter()
    ...
    println!("Time per node range: {:.2}μs - {:.2}μs", 
             min_time.as_micros(), max_time.as_micros());
    ...
    if time_ratio <= MAX_ACCEPTABLE_TIME_RATIO {
        ...
        println!("  Time ratio {} exceeds maximum acceptable ratio {}", 
...
pub(crate) fn test_efficient_data_structures() {
    ...
    println!("1000 set operations completed in: {:.2}ms", operations_time.as_millis());
    ...
    if operations_time.as_millis() <= MAX_OPERATIONS_TIME_MS {
        ...
        println!("  Operations took {}ms, expected < {}ms", 
                 operations_time.as_millis(), MAX_OPERATIONS_TIME_MS);
    }
}
...
pub(crate) fn test_minimal_cloning() {
    ...
    println!("1000 reference operations completed in: {:.2}μs", ref_operations_time.as_micros());
    ...
    if ref_operations_time.as_millis() <= MAX_REF_OPERATIONS_TIME_MS {
        ...
        println!("  Reference operations took {}ms, expected < {}ms", 
                 ref_operations_time.as_millis(), MAX_REF_OPERATIONS_TIME_MS);
    }
}
...
mod tests {
    ...
    fn test_synthetic_cfg_creation() {
        ...
        let total_edges: usize = cfg.nodes.values()
