...
fn test_single_field_struct_layout() {
    ...
    let fields = vec![
        create_test_field(&mut string_table, "value", DataType::Int),
    ];
...
fn test_struct_definition_transformation() {
    ...
    assert!(result.is_ok(), "Struct definition transformation should succeed");
    
    let struct_node = result.unwrap();
    match &struct_node.kind {
        HirKind::Stmt(HirStmt::StructDef { name, fields: def_fields }) => {
...
fn test_struct_creation_transformation() {
    ...
    assert!(result.is_ok(), "Struct creation transformation should succeed");
    
    let (nodes, expr) = result.unwrap();
    assert!(nodes.is_empty(), "Simple struct creation should not produce setup nodes");
...
fn test_field_access_transformation() {
    ...
    let (nodes, expr) = result.unwrap();
    assert!(nodes.is_empty(), "Simple field access should not produce setup nodes");
...
fn test_field_assignment_transformation() {
    ...
    assert!(result.is_ok(), "Field assignment transformation should succeed");
...
fn property_struct_definition_preserves_field_count() {
    for field_count in 0..=5 {
        ...
        assert!(result.is_ok(), "Struct with {} fields should transform successfully", field_count);
        
        let struct_node = result.unwrap();
        match &struct_node.kind {
            HirKind::Stmt(HirStmt::StructDef { fields: def_fields, .. }) => {
                assert_eq!(
                    def_fields.len(),
                    field_count,
...
fn property_struct_creation_preserves_field_values() {
    for field_count in 1..=5 {
        ...
        assert!(result.is_ok(), "Struct creation with {} fields should succeed", field_count);
        
        let (_, expr) = result.unwrap();
        match &expr.kind {
            HirExprKind::StructConstruct { fields: hir_fields, .. } => {
                assert_eq!(
                    hir_fields.len(),
                    field_count,
    ...
    for i in 0..5 {
        ...
        match &nodes[0].kind {
            HirKind::Stmt(HirStmt::Assign { is_mutable, .. }) => {
                assert!(
                    *is_mutable,
                    "Field assignment should always be mutable"
                );
            }
            _ => panic!("Expected Assign statement"),
        }
...
fn property_heap_allocation_creates_valid_nodes() {
    ...
    for (size, alignment) in test_cases {
        ...
        let result = handler.handle_heap_allocation(
            size,
            alignment,
            &mut ctx,
            &TextLocation::default(),
        ...
        assert!(result.is_ok(), "Heap allocation for size {} should succeed", size);
...
fn property_heap_allocation_preserves_parameters() {
    ...
    match &nodes[0].kind {
        HirKind::Stmt(HirStmt::HostCall { args, .. }) => {
            assert_eq!(args.len(), 2, "Allocation should have size and alignment args");
...
fn property_nested_field_offset_is_cumulative() {
    ...
    let single_offset = handler.calculate_nested_field_offset(
        struct_name,
        &[field_a],
        &ctx,
    );
    ...
    assert_eq!(single_offset.unwrap(), 0, "Single field offset should match");
}
    ...
    for (i, node) in nodes.iter().enumerate() {
        match &node.kind {
            HirKind::Stmt(HirStmt::Assign { target, is_mutable, .. }) => {
                ...
                assert!(!*is_mutable, "Destructuring should create immutable bindings");
