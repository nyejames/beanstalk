...

    {
        ...
        if let Err(ref e) = build_result {
            panic!(
                ...
                crate::compiler_frontend::display_messages::format_terse_compiler_messages(e)
                    .join("; ")
            );
        }
        ...
        assert!(
            messages.error_diagnostics().any(|diagnostic| matches!(
                ...
                DiagnosticPayload::InvalidTemplateSlot {
                    reason: InvalidTemplateSlotReason::InsertOutsideParentSlot,
                    ..
                }
            )),
            "expected a targeted slot insertion usage diagnostic, got: {:?}",
            messages
                .error_diagnostics()
