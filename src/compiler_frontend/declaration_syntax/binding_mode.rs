...
//!
//! WHY: binding mode is a syntactic/semantic declaration property (`=`, `~=`, `#=`, future `$=`)
...
/// `#` compile-time constants and planned reactive binding modes.
...
pub enum BindingMode {
    ...
    CompileTimeConstant,
    // Future: reactive binding mode (`$=` / `$Type`).
}

impl BindingMode {
    /// Returns `true` for mutable runtime bindings.
