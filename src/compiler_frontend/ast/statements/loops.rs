...
/// Main entry point for loop statement parsing.
///
/// WHAT: consumes the loop header tokens up to the colon, dispatches to the appropriate
/// loop-kind parser, then parses the body as a function-like block.
/// WHY: loop headers have three distinct syntactic shapes (conditional, range, collection)
/// that must be disambiguated before body parsing so the correct `NodeKind` is produced.
pub fn create_loop(
    ...
    let (parsed_loop_header, body_context) = parse_loop_header(
        &header_tokens,
        context,
        type_interner,
        warnings,
...
fn parse_range_loop_header(
    ...
    if let Some(pipe_binding_split) = parse_pipe_binding_suffix(header_tokens, parser.string_table)?
    {
        ...
        let binding_type = range_binding_type(
            &range,
            parser.type_interner.environment(),
            parser.string_table,
        )?;
        let bindings =
