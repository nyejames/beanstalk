...
pub(crate) fn apply_body_formatter(
    ...
    let process_run =
        |run: Vec<RenderPiece>,
         ...
         string_table: &mut StringTable|
         -> Result<(Vec<RenderPiece>, Vec<CompilerDiagnostic>), CompilerMessages> {
            if run.is_empty() {
            ...
            // 1. Build the opaque-anchor side-table: each non-text piece gets a FormatterAnchorId
            ...
            // 2. Run pre-format whitespace passes (operates directly on structured input).
            let mut output = apply_whitespace_passes_to_input(
                input,
                pre_format_passes,
                run_position,
                string_table,
            ...
            // 4. Run post-format whitespace passes.
            ...
            // 5. Map formatter output back to render pieces using the anchor side-table.
...
fn representative_text_location_for_run(run: &[RenderPiece]) -> SourceLocation {
    ...
    for piece in run {
        match piece {
            ...
            RenderPiece::HeadContent(_) | RenderPiece::LoopControl(_) | RenderPiece::Slot(_) => {}
        }
    }
...
mod tests {
    ...
    use crate::compiler_frontend::ast::templates::template::{
        BodyWhitespacePolicy, Formatter, FormatterResult, Style, TemplateContent,
        TemplateFormatter, TemplateSegmentOrigin,
    };
    use crate::compiler_frontend::ast::templates::template_render_plan::{
        FormatterAnchorId, FormatterInput, FormatterOpaqueKind, FormatterOpaquePiece,
