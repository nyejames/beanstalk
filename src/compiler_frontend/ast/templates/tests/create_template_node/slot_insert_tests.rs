...
fn assert_template_slot_error(
    ...
    expected_reason: InvalidTemplateSlotReason,
) {
    match &diagnostic.payload {
        DiagnosticPayload::InvalidTemplateSlot { reason, .. } => {
            assert_eq!(*reason, expected_reason);
        }
        payload => panic!("expected invalid template slot payload, found {payload:?}"),
    }
}
...
fn rejects_duplicate_default_slot_definitions() {
    ...
    assert_template_slot_error(&error, InvalidTemplateSlotReason::MultipleDefaultSlots);
}
