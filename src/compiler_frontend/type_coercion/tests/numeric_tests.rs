...
fn int_literal(value: i64) -> Expression {
    Expression::int(value, SourceLocation::default(), Ownership::ImmutableOwned)
}

fn float_literal(value: f64) -> Expression {
    Expression::float(value, SourceLocation::default(), Ownership::ImmutableOwned)
}
...
fn float_declaration_rejects_bool_unchanged() {
    ...
    let expr = Expression::bool(true, SourceLocation::default(), Ownership::ImmutableOwned);
    let result = coerce_expression_to_declared_type(expr, &DataType::Float);
