...
pub fn execute_direct_jit_with_capture(
    ...
    let import_object = create_import_object_with_capture(&mut store, &module, &config.io_backend, capture_output)?;
...
pub fn create_import_object(
    ...
    create_import_object_with_capture(store, module, io_backend, false)
}
...
pub fn create_import_object_with_capture(
    ...
    match io_backend {
        IoBackend::Wasix => {
            ...
            setup_wasix_imports_with_io(store, module, capture_output)
        }
...
pub fn create_import_object_with_wasix_native(
    ...
    match io_backend {
        IoBackend::Wasix => {
            ...
            setup_wasix_imports_with_native_support(store, module)
        }
        _ => {
...
/// Set up WASIX imports with configurable I/O redirection
fn setup_wasix_imports_with_io(
    ...
    // Create fd_write function that uses shared memory state
    let fd_write_func = Function::new_typed(store, |fd: i32, iovs_ptr: i32, iovs_len: i32, nwritten_ptr: i32| -> i32 {
        ...
        // Get memory and store from shared state
        let memory = match get_wasix_memory() {
            Some(mem) => mem,
            None => {
                eprintln!("Error: WASIX memory not available - instance not properly initialized");
                return 8; // ENOEXEC
            }
        };
        ...
        let store_ptr = match get_wasix_store() {
            Some(ptr) => ptr,
            None => {
                eprintln!("Error: WASIX store not available - instance not properly initialized");
                return 8; // ENOEXEC
            }
        };
        ...
        match implement_fd_write_with_memory(&memory, store_ref, fd, iovs_ptr as u32, iovs_len as u32, nwritten_ptr as u32) {
            ...
            Err(e) => {
                eprintln!("WASIX fd_write error: {}", e);
                8 // ENOEXEC
            }
        }
    });
...
fn setup_wasix_imports_with_native_support(
    ...
    let wasi_env = wasi_env_builder
        .finalize(store)
        .map_err(|e| CompileError::compiler_error(&format!("Failed to create WASIX environment: {}", e)))?;
    
    // Generate WASIX import object with native function support
    let wasix_imports = wasi_env
        .import_object(store, module)
        .map_err(|e| CompileError::compiler_error(&format!("Failed to create WASIX imports: {}", e)))?;
...
impl JitRuntime {
    ...
    pub fn execute_wasm_with_wasix(
        ...
        // Set up imports based on IO backend
        let import_object = create_import_object_with_wasix_native(&mut self.store, &module, &config.io_backend, &mut self.wasix_registry)?;
