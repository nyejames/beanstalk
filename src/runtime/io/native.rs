...
impl IoInterface for NativeIoBackend {
    ...
    fn write_file(&self, path: &str, content: &str) -> Result<(), CompileError> {
        std::fs::write(path, content)
            .map_err(|e| CompileError::file_error(std::path::Path::new(path), &format!("Failed to write file: {}", e)))
    }
    
    fn read_file(&self, path: &str) -> Result<String, CompileError> {
        std::fs::read_to_string(path)
            .map_err(|e| CompileError::file_error(std::path::Path::new(path), &format!("Failed to read file: {}", e)))
    }
}