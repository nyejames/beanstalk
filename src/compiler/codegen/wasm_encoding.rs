use wasm_encoder::*;
use crate::compiler::mir::mir_nodes::{MIR, MirFunction, Statement, Rvalue, Operand, Constant, Terminator};
...
impl WasmModule {
    ...
    fn initialize_types_from_mir(&mut self, mir: &MIR) -> Result<(), CompileError> {
        ...
        for function_sig in &mir.type_info.function_types {
            let param_types: Vec<ValType> = function_sig.param_types
                .iter()
                .map(|wasm_type| self.wasm_type_to_val_type(wasm_type))
                .collect();
            
            let result_types: Vec<ValType> = function_sig.result_types
                .iter()
                .map(|wasm_type| self.wasm_type_to_val_type(wasm_type))
                .collect();
    ...
    fn initialize_string_constants_from_mir(&mut self, mir: &MIR) -> Result<(), CompileError> {
        ...
        for function in &mir.functions {
            for block in &function.blocks {
                ...
                self.collect_string_constants_from_terminator(&block.terminator, &mut current_offset)?;
            }
        }
    ...
    fn generate_interface_method_types(&mut self, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
    ...
    fn create_interface_function_table(&mut self, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
    ...
    /// Generate vtable data structures in linear memory
    fn generate_vtable_data_structures(&mut self, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
    ...
    pub fn get_interface_method_type_index(&self, interface_id: u32, method_id: u32, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<u32, CompileError> {
        // Find the interface definition
        let interface_def = interface_info.interfaces.get(&interface_id)
            .ok_or_else(|| CompileError::new_thread_panic(
                format!("Interface {} not found", interface_id)
            ))?;
        ...
        let method = interface_def.methods.iter()
            ...
            .ok_or_else(|| CompileError::new_thread_panic(
                format!("Method {} not found in interface {}", method_id, interface_id)
            ))?;
        ...
        return_compiler_error!("Could not determine type index for interface {} method {}", interface_id, method_id);
    }

    /// Calculate vtable offset for a given interface and implementing type
    pub fn calculate_vtable_offset(&self, interface_id: u32, type_id: u32, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<u32, CompileError> {
        let mut offset = 0u32;
        ...
        return_compiler_error!("VTable not found for interface {} and type {}", interface_id, type_id);
    }

    /// Generate interface method index mapping for efficient dispatch
    pub fn create_interface_method_mapping(&self, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> InterfaceMethodMapping {
            ...
            for method in &interface_def.methods {
                ...
                for (_vtable_id, vtable) in &interface_info.vtables {
                    if vtable.interface_id == interface_def.id {
                        ...
                        if let Some(method_index) = interface_def.methods.iter().position(|m| m.id == method.id) {
    ...
    pub fn compile_mir_function_with_interface_support(&mut self, mir_function: &MirFunction, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<u32, CompileError> {
        // Add function signature to function section
        let type_index = self.get_or_create_function_type(mir_function)?;
        self.function_signature_section.function(type_index);
        ...
        for block in &mir_function.blocks {
            self.lower_block_to_wasm_with_interface_support(block, &mut function, &local_map, interface_info)?;
        }
    ...
    pub fn wasm_types_to_val_types(&self, wasm_types: &[WasmType]) -> Vec<ValType> {
        wasm_types.iter().map(|wt| self.wasm_type_to_val_type(wt)).collect()
    }

    /// Validate type compatibility between MIR and WASM
    pub fn validate_type_compatibility(&self, mir_type: &WasmType, expected_wasm_type: ValType) -> Result<(), CompileError> {
        ...
        if actual_wasm_type != expected_wasm_type {
            return_compiler_error!(
                ...
                mir_type, actual_wasm_type, expected_wasm_type
            );
        }
        Ok(())
    }
    ...
    pub fn add_function_signature_from_mir(&mut self, mir_function: &MirFunction) -> Result<u32, CompileError> {
    ...
    /// Validate WASM function signature constraints
    fn validate_function_signature(&self, param_types: &[ValType], result_types: &[ValType]) -> Result<(), CompileError> {
    ...
    fn get_or_create_function_type(&mut self, mir_function: &MirFunction) -> Result<u32, CompileError> {
    ...
    pub fn add_interface_method_signature(&mut self, method: &crate::compiler::mir::mir_nodes::MethodSignature) -> Result<u32, CompileError> {
        // Convert parameter types
        let param_types = self.wasm_types_to_val_types(&method.param_types);
    ...
    pub fn calculate_struct_layout(&self, field_types: &[WasmType]) -> StructLayout {
        ...
        for (field_index, field_type) in field_types.iter().enumerate() {
            ...
            layout.add_field(field_index as u32, current_offset, field_size, field_alignment);
            current_offset += field_size;
        }
        
        // Align the total size to the largest alignment requirement
        let max_alignment = field_types.iter()
    ...
    pub fn create_type_index_mapping(&mut self, mir: &MIR) -> Result<TypeIndexMapping, CompileError> {
    ...
    fn build_local_map(&self, mir_function: &MirFunction) -> Result<HashMap<Place, u32>, CompileError> {
    ...
    fn build_function_locals(&self, mir_function: &MirFunction, local_map: &HashMap<Place, u32>) -> Result<Vec<(u32, ValType)>, CompileError> {
    ...
    fn collect_string_constants_from_statement(&mut self, statement: &Statement, current_offset: &mut u32) -> Result<(), CompileError> {
        match statement {
    ...
    fn collect_string_constants_from_rvalue(&mut self, rvalue: &Rvalue, current_offset: &mut u32) -> Result<(), CompileError> {
        match rvalue {
            Rvalue::Use(operand) => {
    ...
    fn collect_string_constants_from_operand(&mut self, operand: &Operand, current_offset: &mut u32) -> Result<(), CompileError> {
        if let Operand::Constant(Constant::String(string_value)) = operand {
            if !self.string_constant_map.contains_key(string_value) {
                self.string_constant_map.insert(string_value.clone(), *current_offset);
    ...
    fn collect_string_constants_from_terminator(&mut self, terminator: &Terminator, current_offset: &mut u32) -> Result<(), CompileError> {
        match terminator {
    ...
    fn lower_block_to_wasm(&self, block: &crate::compiler::mir::mir_nodes::MirBlock, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    pub fn lower_block_to_wasm_with_interface_support(&self, block: &crate::compiler::mir::mir_nodes::MirBlock, function: &mut Function, local_map: &HashMap<Place, u32>, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
        // Lower each statement in the block with interface support
        for statement in &block.statements {
            match statement {
                Statement::InterfaceCall { interface_id, method_id, receiver, args, destination } => {
                    self.lower_interface_call(*interface_id, *method_id, receiver, args, destination, function, local_map, interface_info)?;
                }
                _ => {
                    self.lower_statement(statement, function, local_map)?;
                }
    ...
    fn generate_function_prologue(&self, mir_function: &MirFunction, function: &mut Function, _local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        ...
        if mir_function.parameters.len() > u32::MAX as usize {
            return_compiler_error!("Function has too many parameters: {}", mir_function.parameters.len());
        }
    ...
    fn generate_function_epilogue(&self, mir_function: &MirFunction, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        ...
        match mir_function.return_types.len() {
            ...
            _ => {
                ...
                return_compiler_error!("Multi-value returns not yet fully implemented for function '{}'", mir_function.name);
            }
        }
    ...
    fn validate_generated_function(&self, mir_function: &MirFunction, _function: &Function) -> Result<(), CompileError> {
        // Validate function signature constraints
        if mir_function.parameters.len() > 1000 {
            return_compiler_error!("Function '{}' has too many parameters: {}", mir_function.name, mir_function.parameters.len());
        }
        
        if mir_function.return_types.len() > 1000 {
            return_compiler_error!("Function '{}' has too many return types: {}", mir_function.name, mir_function.return_types.len());
        }
        ...
        for (i, param_place) in mir_function.parameters.iter().enumerate() {
            ...
            if let Err(_) = self.validate_wasm_val_type(&val_type) {
                return_compiler_error!("Invalid parameter type at index {} in function '{}'", i, mir_function.name);
            }
        }
        ...
        for (i, return_type) in mir_function.return_types.iter().enumerate() {
            ...
            if let Err(_) = self.validate_wasm_val_type(&val_type) {
                return_compiler_error!("Invalid return type at index {} in function '{}'", i, mir_function.name);
            }
        }
    ...
    pub fn lower_statement(&self, statement: &Statement, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        match statement {
            ...
            Statement::Drop { place } => {
                self.lower_drop_statement(place, function, local_map)
            }
    ...
    fn lower_call_statement(&self, func: &Operand, args: &[Operand], destination: &Option<Place>, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    pub fn lower_interface_call(&self, interface_id: u32, method_id: u32, receiver: &Operand, args: &[Operand], destination: &Option<Place>, function: &mut Function, local_map: &HashMap<Place, u32>, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
        // Validate the interface call before lowering with detailed error reporting
        self.validate_interface_call_with_context(interface_id, method_id, receiver, args, interface_info)?;
        ...
        let type_index = self.get_interface_method_type_index(interface_id, method_id, interface_info)?;
    ...
    pub fn get_vtable_offset_in_object(&self, _object_type: &WasmType) -> Result<u32, CompileError> {
    ...
    pub fn calculate_method_offset_in_vtable(&self, interface_id: u32, method_id: u32, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<u32, CompileError> {
        // Find the interface definition
        let interface_def = interface_info.interfaces.get(&interface_id)
            .ok_or_else(|| CompileError::new_thread_panic(
                format!("Interface {} not found", interface_id)
        ...
        // Find the method index within the interface
        let method_index = interface_def.methods.iter()
    ...
    pub fn create_interface_call_error(&self, interface_id: u32, method_id: u32, error_type: &str, context: &str) -> CompileError {
    ...
    pub fn validate_interface_call_with_context(&self, interface_id: u32, method_id: u32, receiver: &Operand, args: &[Operand], interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
        ...
        if !interface_info.interfaces.contains_key(&interface_id) {
            return Err(self.create_interface_call_error(
                ...
                method_id, 
                "Interface not found", 
                &format!("Available interfaces: {:?}", interface_info.interfaces.keys().collect::<Vec<_>>())
            ));
        }
        ...
        if !method_exists {
            return Err(self.create_interface_call_error(
                ...
                &format!("Available methods: {:?}", interface_def.methods.iter().map(|m| m.id).collect::<Vec<_>>())
            ));
        }

        // Validate types
        let receiver_type = self.infer_operand_type(receiver).map_err(|_| {
            self.create_interface_call_error(interface_id, method_id, "Cannot infer receiver type", "Receiver operand type inference failed")
        })?;

        let arg_types: Result<Vec<_>, _> = args.iter().map(|arg| self.infer_operand_type(arg)).collect();
        let arg_types = arg_types.map_err(|_| {
            self.create_interface_call_error(interface_id, method_id, "Cannot infer argument types", "One or more argument types could not be inferred")
        })?;

        self.validate_interface_call_types(interface_id, method_id, &receiver_type, &arg_types, interface_info)
            .map_err(|_| {
                self.create_interface_call_error(
                    interface_id, 
                    method_id, 
                    "Type validation failed", 
                    &format!("Receiver type: {:?}, Argument types: {:?}", receiver_type, arg_types)
                )
            })?;
    ...
    pub fn load_receiver_vtable_pointer(&self, receiver: &Operand, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    pub fn load_method_function_index(&self, interface_id: u32, method_id: u32, function: &mut Function, interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
        // Calculate method offset in vtable
        let method_offset = self.calculate_method_offset_in_vtable(interface_id, method_id, interface_info)?;
    ...
    pub fn validate_interface_call_types(&self, interface_id: u32, method_id: u32, receiver_type: &WasmType, arg_types: &[WasmType], interface_info: &crate::compiler::mir::mir_nodes::InterfaceInfo) -> Result<(), CompileError> {
        // Find the interface definition
        let interface_def = interface_info.interfaces.get(&interface_id)
            .ok_or_else(|| CompileError::new_thread_panic(
                format!("Interface {} not found for type checking", interface_id)
            ))?;
        ...
        let method = interface_def.methods.iter()
            .find(|m| m.id == method_id)
            .ok_or_else(|| CompileError::new_thread_panic(
                format!("Method {} not found in interface {} for type checking", method_id, interface_id)
            ))?;
        ...
        if !method.param_types.is_empty() {
            ...
            if receiver_type != expected_receiver_type {
                return_compiler_error!(
                    ...
                    expected_receiver_type, receiver_type
                );
            }
        }
        ...
        if arg_types.len() != expected_arg_types.len() {
            return_compiler_error!(
                ...
                expected_arg_types.len(), arg_types.len()
            );
        }
    ...
    pub fn lower_rvalue(&self, rvalue: &Rvalue, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        match rvalue {
            Rvalue::Use(operand) => {
            ...
            Rvalue::MemorySize => {
                self.lower_memory_size(function)
            }
            
            Rvalue::MemoryGrow { pages } => {
                self.lower_memory_grow(pages, function, local_map)
            }
            
            Rvalue::InterfaceCall { .. } => {
                return_compiler_error!("InterfaceCall rvalue lowering not yet implemented - will be added in task 11");
            }
        }
    }

    /// Lower binary operations to WASM arithmetic instructions (i32.add, i32.mul, etc.)
    fn lower_binary_op(&self, op: &crate::compiler::mir::mir_nodes::BinOp, left: &Operand, right: &Operand, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        ...
        match op {
            ...
            BinOp::Div => { function.instruction(&Instruction::I32DivS); } // Signed division
            BinOp::Rem => { function.instruction(&Instruction::I32RemS); } // Signed remainder
            
            // Bitwise operations
            BinOp::BitAnd => { function.instruction(&Instruction::I32And); }
            BinOp::BitOr => { function.instruction(&Instruction::I32Or); }
            BinOp::BitXor => { function.instruction(&Instruction::I32Xor); }
            BinOp::Shl => { function.instruction(&Instruction::I32Shl); }
            BinOp::Shr => { function.instruction(&Instruction::I32ShrS); } // Signed right shift
            
            // Comparison operations
            BinOp::Eq => { function.instruction(&Instruction::I32Eq); }
            BinOp::Ne => { function.instruction(&Instruction::I32Ne); }
            BinOp::Lt => { function.instruction(&Instruction::I32LtS); } // Signed less than
            BinOp::Le => { function.instruction(&Instruction::I32LeS); } // Signed less equal
            BinOp::Gt => { function.instruction(&Instruction::I32GtS); } // Signed greater than
            BinOp::Ge => { function.instruction(&Instruction::I32GeS); } // Signed greater equal
            
            // Logical operations (implemented as short-circuiting control flow)
            BinOp::And | BinOp::Or => {
                return_compiler_error!("Logical And/Or operations should be lowered as control flow, not binary ops");
            }
        }
    ...
    fn lower_unary_op(&self, op: &crate::compiler::mir::mir_nodes::UnOp, operand: &Operand, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    fn lower_cast(&self, source: &Operand, target_type: &WasmType, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        // Load source operand
        ...
        match (source_type.clone(), target_type) {
            ...
            _ => {
                return_compiler_error!("Unsupported cast from {:?} to {:?}", source_type, target_type);
            }
        }
    }

    /// Lower array creation with linear memory allocation
    fn lower_array_creation(&self, elements: &[Operand], element_type: &WasmType, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        ...
        for (i, element) in elements.iter().enumerate() {
            ...
            match element_type {
                WasmType::I32 => { function.instruction(&Instruction::I32Store(MemArg { offset: 0, align: 2, memory_index: 0 })); }
                WasmType::I64 => { function.instruction(&Instruction::I64Store(MemArg { offset: 0, align: 3, memory_index: 0 })); }
                WasmType::F32 => { function.instruction(&Instruction::F32Store(MemArg { offset: 0, align: 2, memory_index: 0 })); }
                WasmType::F64 => { function.instruction(&Instruction::F64Store(MemArg { offset: 0, align: 3, memory_index: 0 })); }
                WasmType::ExternRef | WasmType::FuncRef => { function.instruction(&Instruction::I32Store(MemArg { offset: 0, align: 2, memory_index: 0 })); }
            }
        }
    ...
    fn lower_struct_creation(&self, fields: &[(u32, Operand)], _struct_type: u32, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    fn lower_memory_grow(&self, pages: &Operand, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    fn fold_binary_op_constants(&self, op: &crate::compiler::mir::mir_nodes::BinOp, left: &Constant, right: &Constant) -> Option<Constant> {
        use crate::compiler::mir::mir_nodes::BinOp;
    ...
    fn fold_unary_op_constant(&self, op: &crate::compiler::mir::mir_nodes::UnOp, operand: &Constant) -> Option<Constant> {
        use crate::compiler::mir::mir_nodes::UnOp;
    ...
    pub fn lower_operand(&self, operand: &Operand, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        match operand {
            Operand::Copy(place) => {
    ...
    pub fn lower_constant(&self, constant: &Constant, function: &mut Function) -> Result<(), CompileError> {
        match constant {
            Constant::I32(value) => {
            ...
            Constant::String(string_value) => {
                ...
                if let Some(&offset) = self.string_constant_map.get(string_value) {
                    ...
                    return_compiler_error!("String constant '{}' not found in string constant map", string_value);
                }
            }
    ...
    pub fn resolve_place_load(&self, place: &Place, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        match place {
            Place::Local { index, wasm_type: _ } => {
                // Map MIR local to WASM local index using local_map
                let wasm_local_index = local_map.get(place)
                    .copied()
                    .unwrap_or(*index);
            ...
            Place::Global { index, wasm_type } => {
                ...
                if *index >= self.global_count {
                    return_compiler_error!("Global index {} exceeds global_count {}", index, self.global_count);
                }
    ...
    pub fn resolve_place_store(&self, place: &Place, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        match place {
            Place::Local { index, wasm_type: _ } => {
                // Map MIR local to WASM local index using local_map
                let wasm_local_index = local_map.get(place)
    ...
    pub fn build_local_index_mapping(&mut self, mir_function: &MirFunction) -> Result<HashMap<Place, u32>, CompileError> {
        let mut local_map = HashMap::new();
    ...
    pub fn add_global_index_mapping(&mut self, _mir_global_id: u32, place: &Place) -> Result<u32, CompileError> {
        match place {
            Place::Global { index, wasm_type } => {
    ...
    fn resolve_memory_load(&self, base: &crate::compiler::mir::place::MemoryBase, offset: &crate::compiler::mir::place::ByteOffset, size: &crate::compiler::mir::place::TypeSize, function: &mut Function, _local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        ...
        match base {
            MemoryBase::LinearMemory => {
                ...
                match size {
                    ...
                    TypeSize::Short => {
                        function.instruction(&Instruction::I32Load16U(MemArg { offset: 0, align: 1, memory_index: 0 }));
                    }
                    TypeSize::Word => {
                    ...
                    TypeSize::Custom { bytes, alignment } => {
                        ...
                        if *bytes <= 4 {
                            ...
                            function.instruction(&Instruction::I32Load(MemArg { offset: 0, align: align_log2, memory_index: 0 }));
                        } else {
                            let align_log2 = alignment.trailing_zeros().min(3);
                            function.instruction(&Instruction::I64Load(MemArg { offset: 0, align: align_log2, memory_index: 0 }));
                        }
                    }
                }
                Ok(())
            ...
            MemoryBase::Stack => {
                ...
                return_compiler_error!("Stack-based memory should use local operations, not memory loads");
            }
            
            MemoryBase::Heap { alloc_id: _, size: _alloc_size } => {
    ...
    fn resolve_memory_store(&self, base: &crate::compiler::mir::place::MemoryBase, offset: &crate::compiler::mir::place::ByteOffset, size: &crate::compiler::mir::place::TypeSize, function: &mut Function, _local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        use crate::compiler::mir::place::{MemoryBase, TypeSize};
        ...
        match base {
            MemoryBase::LinearMemory => {
                ...
                match size {
                    TypeSize::Byte => {
                        function.instruction(&Instruction::I32Store8(MemArg { offset: 0, align: 0, memory_index: 0 }));
                    }
                    TypeSize::Short => {
                        function.instruction(&Instruction::I32Store16(MemArg { offset: 0, align: 1, memory_index: 0 }));
                    }
                    TypeSize::Word => {
                        function.instruction(&Instruction::I32Store(MemArg { offset: 0, align: 2, memory_index: 0 }));
                    }
                    ...
                    TypeSize::Custom { bytes, alignment } => {
                        ...
                        if *bytes <= 4 {
                            let align_log2 = alignment.trailing_zeros().min(2);
                            function.instruction(&Instruction::I32Store(MemArg { offset: 0, align: align_log2, memory_index: 0 }));
                        } else {
                            let align_log2 = alignment.trailing_zeros().min(3);
                            function.instruction(&Instruction::I64Store(MemArg { offset: 0, align: align_log2, memory_index: 0 }));
                        }
                    }
                }
                Ok(())
            ...
            MemoryBase::Stack => {
                // Stack allocations should be handled as locals, not memory operations
                return_compiler_error!("Stack-based memory should use local operations, not memory stores");
            }
    ...
    fn resolve_projection_load(&self, base: &Place, elem: &crate::compiler::mir::place::ProjectionElem, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        use crate::compiler::mir::place::{ProjectionElem, FieldSize};
        
        match elem {
            ProjectionElem::Field { index: _field_index, offset, size } => {
                    ...
                    FieldSize::WasmType(wasm_type) => {
                        match wasm_type {
                            ...
                            crate::compiler::mir::place::WasmType::F32 => {
                                function.instruction(&Instruction::F32Load(MemArg { offset: 0, align: 2, memory_index: 0 }));
                            }
                            crate::compiler::mir::place::WasmType::F64 => {
                                function.instruction(&Instruction::F64Load(MemArg { offset: 0, align: 3, memory_index: 0 }));
                            }
                            crate::compiler::mir::place::WasmType::ExternRef | crate::compiler::mir::place::WasmType::FuncRef => {
                                function.instruction(&Instruction::I32Load(MemArg { offset: 0, align: 2, memory_index: 0 }));
                            }
                        }
                    }
    ...
    fn resolve_projection_store(&self, base: &Place, elem: &crate::compiler::mir::place::ProjectionElem, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        use crate::compiler::mir::place::{ProjectionElem, FieldSize};
                ...
                match size {
                    ...
                    FieldSize::WasmType(wasm_type) => {
                        match wasm_type {
                            ...
                            crate::compiler::mir::place::WasmType::F32 => {
                                function.instruction(&Instruction::F32Store(MemArg { offset: 0, align: 2, memory_index: 0 }));
                            }
                            crate::compiler::mir::place::WasmType::F64 => {
                                function.instruction(&Instruction::F64Store(MemArg { offset: 0, align: 3, memory_index: 0 }));
                            }
                            crate::compiler::mir::place::WasmType::ExternRef | crate::compiler::mir::place::WasmType::FuncRef => {
                                function.instruction(&Instruction::I32Store(MemArg { offset: 0, align: 2, memory_index: 0 }));
                            }
    ...
    fn validate_local_type(&self, local_index: u32, _expected_type: ValType) -> Result<(), CompileError> {
        ...
        if local_index > self.local_count {
            return_compiler_error!("Local index {} exceeds local_count {}", local_index, self.local_count);
        }
        Ok(())
    }

    /// Validate that a global index has the expected type
    fn validate_global_type(&self, global_index: u32, _expected_type: ValType) -> Result<(), CompileError> {
        // In a full implementation, this would check against the global section types
        // For now, we'll just validate that the index is reasonable
        if global_index >= self.global_count {
            return_compiler_error!("Global index {} exceeds global_count {}", global_index, self.global_count);
        }
        Ok(())
    }
    ...
    pub fn add_memory_export(&mut self, name: &str, memory_index: u32) {
        self.export_section.export(name, ExportKind::Memory, memory_index);
    }

    /// Add a table export
    pub fn add_table_export(&mut self, name: &str, table_index: u32) {
        self.export_section.export(name, ExportKind::Table, table_index);
    }

    /// Add string constant for testing
    pub fn add_string_constant_for_test(&mut self, string_value: String, offset: u32) {
        if !self.string_constant_map.contains_key(&string_value) {
            self.string_constant_map.insert(string_value.clone(), offset);
    ...
    pub fn lower_terminator(&self, terminator: &Terminator, function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
        match terminator {
            Terminator::Goto { target, label_depth } => {
            ...
            Terminator::If { condition, then_block, else_block, wasm_if_info } => {
                self.lower_if_terminator(condition, *then_block, *else_block, wasm_if_info, function, local_map)
            }
            ...
            Terminator::ConditionalJump(_then_block, _else_block) => {
                ...
                return_compiler_error!("ConditionalJump terminator requires condition operand. Use If terminator instead.");
            }
            
            Terminator::Switch { discriminant, targets, default, br_table_info } => {
                self.lower_switch_terminator(discriminant, targets, *default, br_table_info, function, local_map)
            }
            ...
            Terminator::Unreachable => {
                self.lower_unreachable_terminator(function)
            }
            
            Terminator::Loop { target, loop_header, loop_info } => {
                self.lower_loop_terminator(*target, *loop_header, loop_info, function, local_map)
            }
            
            Terminator::Block { inner_blocks, result_type, exit_target } => {
                self.lower_block_terminator(inner_blocks, result_type, *exit_target, function, local_map)
            }
        }
    }

    /// Lower Terminator::Goto to WASM br instruction with correct label depths
    fn lower_goto_terminator(&self, _target: u32, label_depth: u32, function: &mut Function) -> Result<(), CompileError> {
    ...
    fn lower_return_terminator(&self, values: &[Operand], function: &mut Function, local_map: &HashMap<Place, u32>) -> Result<(), CompileError> {
    ...
    pub fn generate_nested_control_flow(
        ...
        for (i, terminator) in terminators.iter().enumerate() {
            self.lower_terminator_with_nesting(terminator, &mut label_manager, function, local_map, i)?;
        }
        ...
        match terminator {
            ...
            Terminator::If { condition, then_block, else_block, wasm_if_info } => {
                ...
                self.lower_if_terminator_with_nesting(
                    condition, *then_block, *else_block, wasm_if_info, 
                    label_manager, function, local_map
                )?;
            ...
            Terminator::Goto { target, label_depth: _ } => {
    ...
    fn lower_if_terminator_with_nesting(
        ...
        let then_depth = label_manager.calculate_branch_depth(then_block).unwrap_or(1);
        ...
        if wasm_if_info.has_else {
            ...
            let else_depth = label_manager.calculate_branch_depth(else_block).unwrap_or(1);
    ...
    pub fn validate_control_flow_structure(
        ...
        for (i, terminator) in terminators.iter().enumerate() {
            if let Err(error) = self.validate_terminator_structure(terminator, &mut label_manager, i, function_name) {
        ...
        if label_manager.get_current_depth() != 0 {
            validation_errors.push(format!(
                ...
                function_name, label_manager.get_current_depth()
            ));
        }
    ...
    fn validate_terminator_structure(
        ...
        match terminator {
            Terminator::Block { inner_blocks, result_type: _, exit_target } => {
                ...
                if label_manager.get_label_depth(*exit_target).is_none() {
                    return Err(format!("Invalid exit target {} in block at terminator {}", exit_target, terminator_index));
                }
            ...
            Terminator::Loop { target, loop_header, loop_info } => {
                ...
                if *target == *loop_header && !loop_info.has_continues {
                    return Err(format!("Loop back-edge without continue support at terminator {}", terminator_index));
                }
            ...
            Terminator::If { condition: _, then_block, else_block, wasm_if_info } => {
                // Validate if structure
                if *then_block == *else_block && wasm_if_info.has_else {
                    return Err(format!("If with same then/else blocks at terminator {}", terminator_index));
                }
            ...
            Terminator::Switch { discriminant: _, targets, default, br_table_info } => {
                // Validate switch structure
                if targets.is_empty() {
                    return Err(format!("Switch with no targets at terminator {}", terminator_index));
                }
                
                if br_table_info.is_dense && br_table_info.max_target < br_table_info.min_target {
                    return Err(format!("Invalid br_table range at terminator {}", terminator_index));
                }
                
                // Validate that default is not in targets (would be redundant)
                if targets.contains(default) {
                    return Err(format!("Default target {} appears in targets list at terminator {}", default, terminator_index));
                }
            }
            ...
            Terminator::Goto { target, label_depth } => {
                ...
                if *label_depth > label_manager.get_current_depth() {
                    return Err(format!("Invalid label depth {} (current depth {}) at terminator {}", 
                        label_depth, label_manager.get_current_depth(), terminator_index));
                }
                
                // Check if target is reachable (if we have the information)
                if let Some(target_depth) = label_manager.get_label_depth(*target) {
                    let calculated_depth = label_manager.get_current_depth().saturating_sub(target_depth);
                    if *label_depth != calculated_depth {
                        return Err(format!("Label depth mismatch: expected {}, got {} at terminator {}", 
    ...
    pub fn create_memory_layout_manager(&mut self, mir: &MIR) -> Result<MemoryLayoutManager, CompileError> {
    ...
    pub fn populate_static_data_section(&mut self, layout_manager: &MemoryLayoutManager) -> Result<(), CompileError> {
    ...
    pub fn calculate_struct_field_layout(&self, field_types: &[WasmType], struct_id: u32) -> Result<StructFieldLayout, CompileError> {
    ...
    pub fn enhance_memory_section(&mut self, layout_manager: &MemoryLayoutManager) -> Result<(), CompileError> {
    ...
    pub fn populate_data_section_with_layout(&mut self, layout_manager: &MemoryLayoutManager) -> Result<(), CompileError> {
    ...
    pub fn setup_heap_allocation_support(&mut self, layout_manager: &MemoryLayoutManager) -> Result<HeapAllocator, CompileError> {
    ...
    fn add_heap_management_functions(&mut self, heap_allocator: &mut HeapAllocator) -> Result<(), CompileError> {
        ...
        self.export_section.export("malloc", ExportKind::Func, malloc_func_index);
        self.export_section.export("free", ExportKind::Func, free_func_index);
        ...
        for field in &layout.fields {
            if field.offset % field.alignment != 0 {
                return_compiler_error!(
                    ...
                    field.index, field.offset, field.alignment
                );
            }
        }
        ...
        for i in 1..sorted_fields.len() {
            ...
            if prev_end > curr_field.offset {
                return_compiler_error!(
                    ...
                    curr_field.index, curr_field.offset, prev_field.index, prev_end
                );
            }
        }
    ...
    pub fn get_memory_layout_stats(&self, layout_manager: &MemoryLayoutManager) -> MemoryLayoutStats {
        MemoryLayoutStats {
            total_static_size: layout_manager.get_total_static_size(),
            string_constants_size: self.string_constants.iter().map(|s| s.len() as u32 + 1).sum(),
            ...
            total_memory_pages: (layout_manager.get_total_static_size() + layout_manager.get_heap_size() + WASM_PAGE_SIZE - 1) / WASM_PAGE_SIZE,
...
impl TypeIndexMapping {
    ...
    pub fn add_interface_method_type(&mut self, interface_id: u32, method_id: u32, type_index: u32) {
        self.interface_method_types.insert((interface_id, method_id), type_index);
    }
    ...
    pub fn get_interface_method_type(&self, interface_id: u32, method_id: u32) -> Option<u32> {
        self.interface_method_types.get(&(interface_id, method_id)).copied()
    }
}
...
impl InterfaceMethodMapping {
    ...
    pub fn add_method_implementation(&mut self, interface_id: u32, method_id: u32, type_id: u32, function_index: u32) {
        self.method_implementations.insert((interface_id, method_id, type_id), function_index);
    ...
    pub fn get_method_implementation(&self, interface_id: u32, method_id: u32, type_id: u32) -> Option<u32> {
        self.method_implementations.get(&(interface_id, method_id, type_id)).copied()
    }
...
impl BlockLabelManager {
    ...
    pub fn enter_control_frame(&mut self, frame_type: ControlFrameType, block_id: Option<u32>) -> u32 {
...
impl MemoryLayoutManager {
    ...
    /// Add string constants using existing fields
    pub fn add_string_constants(&mut self, strings: &[String], string_map: &HashMap<String, u32>) -> Result<(), CompileError> {
    ...
    /// Calculate global data layout
    pub fn calculate_global_data_layout(&mut self, globals: &HashMap<u32, Place>) -> Result<(), CompileError> {
    ...
    fn get_next_available_offset(&self) -> u32 {
        let string_end = self.string_layout.iter()
            .map(|s| s.offset + s.size)
            .max()
            .unwrap_or(0);
        
        let global_end = self.global_layout.iter()
    ...
    pub fn get_string_layout(&self) -> &[StringInfo] { &self.string_layout }
    pub fn get_global_layout(&self) -> &[GlobalInfo] { &self.global_layout }
    pub fn get_struct_layout(&self) -> &[StructInfo] { &self.struct_layout }
    pub fn get_vtable_layout(&self) -> &[VTableInfo] { &self.vtable_layout }
    pub fn get_heap_info(&self) -> &HeapInfo { &self.heap_info }
    
    /// Get size information
    pub fn get_total_static_size(&self) -> u32 { self.total_static_size }
    pub fn get_heap_size(&self) -> u32 { self.heap_info.size }
...
impl StructFieldLayout {
    ...
    pub fn add_field(&mut self, index: u32, offset: u32, size: u32, alignment: u32, wasm_type: WasmType) {
