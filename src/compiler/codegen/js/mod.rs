...
impl<'hir> JsEmitter<'hir> {
    ...
    pub fn emit_block(&mut self, block_id: BlockId) -> Result<(), CompilerError> {
        ...
        let block = match self.blocks.get(&block_id) {
            Some(block) => *block,
            None => {
                // Block not found - this is a compiler bug
    ...
    for func_node in &hir.functions {
        match &func_node.kind {
            ...
            _ => {
                ...
                // If we encounter a non-statement, this is a compiler bug
                return Err(CompilerError::compiler_error(
                    "JavaScript backend: Function node is not a statement. This is a compiler bug - HIR should only contain statement nodes in the functions list.",
                ));
            }
        }
    }
