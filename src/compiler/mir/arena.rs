...
mod tests {
    ...
    fn test_arena_allocation() {
        ...
        let obj1 = arena.alloc(TestStruct {
            value: 42,
            data: vec![1, 2, 3],
        });
        
        let obj2 = arena.alloc(TestStruct {
            value: 84,
            data: vec![4, 5, 6],
        });
        
        // Check that objects are accessible
        assert_eq!(obj1.value, 42);
        assert_eq!(obj2.value, 84);
        assert_eq!(obj1.data, vec![1, 2, 3]);
        assert_eq!(obj2.data, vec![4, 5, 6]);
    }
    ...
    fn test_arena_slice_allocation() {
        ...
        let values = vec![
            TestStruct { value: 1, data: vec![1] },
            TestStruct { value: 2, data: vec![2] },
            TestStruct { value: 3, data: vec![3] },
        ];
    ...
    fn test_pool_max_size() {
        ...
        // Create multiple objects and put them in the pool
        let obj1 = pool.get(); // Creates new object (pool empty)
        let obj2 = pool.get(); // Creates new object (pool empty)
        let obj3 = pool.get(); // Creates new object (pool empty)
        
        pool.put(obj1); // Pool size: 1
        pool.put(obj2); // Pool size: 2
        pool.put(obj3); // Pool size: 2 (should not exceed max_size)
