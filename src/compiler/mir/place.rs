...
mod tests {
    ...
    fn test_place_local_creation() {
        ...
        assert_eq!(place.wasm_type(), WasmType::I64);
        assert_eq!(place.load_instruction_count(), 1);
    }
    
    #[test]
    fn test_place_global_creation() {
        let data_type = DataType::Float(crate::compiler::datatypes::Ownership::ImmutableOwned(false));
        let place = Place::global(0, &data_type);
        
        assert!(place.is_wasm_global());
        assert_eq!(place.wasm_type(), WasmType::F64);
        assert_eq!(place.store_instruction_count(), 1);
    }
    
    #[test]
    fn test_place_memory_creation() {
        let place = Place::memory(1024, TypeSize::Word);
        
        assert!(place.requires_memory_access());
        assert_eq!(place.load_instruction_count(), 2);
        assert_eq!(place.store_instruction_count(), 2);
    }
    ...
    fn test_place_projection() {
        ...
        assert_eq!(projected.load_instruction_count(), 3); // base + offset + add
    }
    
    #[test]
    fn test_place_manager() {
        ...
        assert!(local.is_wasm_local());
        assert!(global.is_wasm_global());
        ...
        assert_eq!(global_types.len(), 1);
        assert_eq!(global_types[0], WasmType::F64);
    }
    ...
    fn test_stack_operations() {
        ...
        assert_eq!(load_ops.len(), 1);
        assert_eq!(load_ops[0].stack_delta, 1);
        assert_eq!(load_ops[0].wasm_type, WasmType::I64);
        
        let store_ops = place.generate_store_operations();
        assert_eq!(store_ops.len(), 1);
        assert_eq!(store_ops[0].stack_delta, -1);
    }
}