...
impl LifetimeAnalyzer {
    ...
    pub fn sync_with_ir_lifetime_analysis(&self, ir: &mut MIR) {
        ...
        for &variable_id in self.block_variable_usage.values().flat_map(|usage| usage.iter()) {
            all_variables.insert(variable_id);
        }
    ...
    pub fn detect_borrow_violations(&self) -> Vec<BorrowViolation> {
        ...
        for (&variable_id, &last_use_block) in &self.variable_last_use {
            if let Some(_definition_block) = self.get_variable_definition_block(variable_id) {
                ...
                let usage_blocks: Vec<_> = self.block_variable_usage
                    .iter()
                    .filter(|(_, usage)| usage.contains(&variable_id))
                    .map(|(&block_id, _)| block_id)
                    .collect();
                
                if usage_blocks.len() > 2 { // Definition + multiple uses
                    violations.push(BorrowViolation {
                        variable_id,
                        violation_type: BorrowViolationType::PotentialUseAfterMove,
                        block_id: last_use_block,
                        description: format!("Variable {} used across {} blocks", variable_id, usage_blocks.len()),
                    });
                }
            }
        }
    ...
    pub fn clear_block_tracking(&mut self, block_id: u32) {
        ...
        self.variable_last_use.retain(|_, &mut last_block| last_block != block_id);
    }
    ...
    pub fn enter_block(&mut self, block_id: u32) {
        ...
        self.block_variable_definitions.insert(block_id, HashSet::new());
    }
...
pub fn finalize_lifetime_analysis(ir: &mut MIR, analyzer: &LifetimeAnalyzer) -> Result<(), CompileError> {
    ...
    for violation in violations {
        eprintln!("Warning: Potential borrow violation for variable {}: {}", 
    ...
    let terminator = match control_flow_type {
        ControlFlowType::Sequential => {
            if let Some(next_block) = target_blocks.first() {
                Terminator::Goto { target: *next_block, label_depth: 0 }
            } else {
...
impl BlockBuilder {
    ...
    pub fn create_block(&mut self, parent_id: Option<u32>) -> u32 {
        ...
        if let Some(parent) = parent_id {
            self.block_relationships.entry(parent).or_insert_with(Vec::new).push(block_id);
        }
    ...
    pub fn finalize_block(&mut self, block_id: u32, control_flow: ControlFlowType, targets: Vec<u32>) -> Result<(), CompileError> {
    ...
    pub fn get_child_blocks(&self, parent_id: u32) -> Vec<u32> {
        self.block_relationships.get(&parent_id).cloned().unwrap_or_default()
    }
}
...
mod tests {
    ...
    fn test_lifetime_analyzer_basic_functionality() {
        ...
        // Verify tracking
        assert_eq!(analyzer.get_variable_definition_block(variable_id), Some(block1));
    ...
    fn test_borrow_violation_detection() {
        ...
        assert_eq!(violations[0].violation_type, BorrowViolationType::PotentialUseAfterMove);
    }
