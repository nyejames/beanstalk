...
use crate::compiler::borrow_checker::types::{BorrowChecker, Loan, BorrowKind};
use crate::{
    create_multiple_mutable_borrows_error,
    create_shared_mutable_conflict_error, create_use_after_move_error,
    create_move_while_borrowed_error
};
        ...
        for loan2 in active_borrows.iter().skip(i + 1) {
            if loan1.conflicts_with(loan2) {
                ...
                if conflict_exists_on_all_paths(checker, node.id, &loan1.place, &loan2.place) {
                    if let Some(error) = report_borrow_conflict(checker.string_table, loan1, loan2, &node.location) {
                        errors.push(error);
                    }
                }
            }
...
fn conflict_exists_on_all_paths(
    ...
    for &pred_id in &predecessors {
        if let Some(pred_node) = checker.cfg.nodes.get(&pred_id) {
            let has_conflicting_borrows = pred_node.borrow_state
                .active_borrows
                .values()
                .any(|loan1| {
                    pred_node.borrow_state
                        .active_borrows
                        .values()
                        .any(|loan2| {
                            loan1.id != loan2.id
