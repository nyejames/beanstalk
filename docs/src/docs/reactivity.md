...
There are no implicit reactive variables, no hidden schedulers, and no global UI state. A value only becomes reactive when it is *captured reactively* by a template.
