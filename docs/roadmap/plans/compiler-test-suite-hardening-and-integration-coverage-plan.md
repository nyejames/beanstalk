...
STATUS: active
CURRENT_SLICE: Phase 14C15q TIR production head-chain overlay ownership review
LAST_ACCEPTED_COMMIT: `4335023aa` (Phase 14C15p code)
...
- code: all reviewed groups through the obsolete single-pair slot-overlay test seam are complete; its three self-tests, final-caller helper, re-export, and stale future-state comment are removed; the four production head-chain overlay tests are next
ACCEPTANCE_CRITERIA:
...
VALIDATION_STATE:
- `just validate`: passed; cross-target Clippy, 3,346 Rust tests, 1,793 integration executions, docs check, and 28 benchmark cases
- Phase 14C15p focused group: passed; 56 slot-composition tests
...
STOP_REASON: none
NEXT_RESUME_ACTION: launch the four-test TIR production head-chain overlay group through Ollama
...
| Phase 14C5 AST fallible-handling units | `d96a5f1c3` | Accepted | Eight reason-only catch/fallback diagnostics removed in favor of Phase 4C7 structured-reason primaries; 37 AST-shape, scope, terminality, assignment-target, EOF/delimiter, arity/type, and catch-placement owners retained; 3,503 Rust tests and 1,793 integration executions |
...
| Phase 14C14p new-project scaffold and prompt units | `4ff76e04d` | Accepted | Fifteen repeated conflict, preflight-decision, generated-file, `.gitignore`, dev-rule, and scripted-prompt cases were consolidated into labelled owners; the full scaffold/report test now owns every path and empty disposition, generated bytes share one exact owner, and absent `.gitignore` decline gained explicit coverage; 17 scaffold and four shared-prompt owners remain; 3,421 Rust tests and 1,793 integration executions |
...
| `create_project_modules_tests::detects_duplicate_config_keys` | Duplicate explicit config keys are rejected | `config_duplicate_private_explicit_rejected` (boundary of `language.config.duplicate_private_keys_rejected`) | `detects_duplicate_top_level_config_constants` retains `InvalidConfigReason::DuplicateKey`; authored-path remapping has its own focused owner | `0828bb7ea` |
| `create_project_modules_tests::accepts_config_private_key_referencing_explicit_const` | A later config key may fold from an earlier config constant | `config_private_const_refs_success` | `accepts_config_local_reference_to_earlier_private_const` is the identical folded-state unit owner | `0828bb7ea` |
| `create_project_modules_tests::{rejects_core_string_key_with_int_value,rejects_core_string_key_with_char_value,rejects_backend_bool_key_with_int_value,rejects_backend_string_key_with_bool_value}` | Config schema rejects values with the wrong primitive shape | `config_wrong_shape_string_rejected` (`language.config.value_type_mismatch_rejected`) owns the source-visible family | `rejects_core_string_key_with_bool_value` and `rejects_backend_bool_key_with_string_value` retain exact string/bool expected-shape payloads | `0828bb7ea` |
| `create_project_modules_tests::rejects_closed_string_set_config_key_with_unsupported_value` | Closed-set config values reject unsupported strings | `config_invalid_project_value_rejected` (`language.config.invalid_project_value_rejected`) | focused string/bool shape units retain the shared typed payload family | `0828bb7ea` |
| `create_project_modules_tests::rejects_package_folders_with_mixed_collection` | Heterogeneous collection literals are rejected while parsing config | `mixed_collection_literal_rejected` (`language.collections.mixed_literal_rejected`) | none; the removed unit asserted only that some ordinary AST diagnostic existed | `0828bb7ea` |
| `compile_project_frontend_tests::provider_backed_import_participates_in_visible_name_collisions` | Provider-backed grouped imports participate in the ordinary visible-name collision rule | `source_import_alias_collision_with_local_rejected` (`language.imports.import_name_collision_rejected`) plus external boundary cases including `external_import_alias_collision_with_source_rejected` | provider grouped/namespace compile and cache tests retain provider registration and lowering facts | `2aa652d29` |
...
| `build_cleanup_tests::cleanup_manifest_diff_handles_small_static_site_route_and_asset_layout` | Manifest diff removes stale nested routes and byte assets while retaining current outputs | `cleanup_manifest_diff_removes_stale_managed_files` and `cleanup_manifest_diff_removes_stale_tracked_byte_assets_from_v2_manifest` | current-path preservation and byte-asset manifest ownership remain separate focused facts | `176c0ea95` |
...
| `watch_tests::{directory_scope_watches_js_files_under_entry_root,directory_scope_watches_js_files_under_package_folders}` | Derived directory watch scope includes JavaScript below the entry root and nested package folders while excluding generated output | `watch_tests::directory_scope_watches_config_entry_root_and_package_folders`, strengthened to cover config, Beanstalk, JavaScript, nested package, unrelated target, and output paths | scanner, ignore-rule, exact-file, recursive-directory, and fingerprint owners remain separate | `fce3e9e82` |
...
| Fifteen same-family tests in `new_html_project::{scaffold_tests,prompt_tests}` consolidated in Phase 14C14p | Scaffold conflict inventory, force/non-empty decisions, exact generated files, report/existence shape, `.gitignore` decisions and rule matching, and scripted prompt normalization/defaults | 17 scaffold and four prompt owners, including the exact conflict set, full scaffold/report, exact templates, create/append/already-present `.gitignore` dispositions, end-to-end no-write/force seams, and labelled prompt tables | direct `write_scaffold` IO remains distinct from end-to-end command orchestration; exact error, escaping, and force-preservation owners remain separate | `4ff76e04d` |
| Four per-type TIR store-ID round-trip tests consolidated in Phase 14C15a | Compact `u32` store IDs round-trip through typed constructors and accessors | `ids_tests::typed_ids_round_trip_through_u32_bound`, labelled for all four store ID types at `u32::MAX` | distinct-type equality and exact display owners remain separate | `8e89329ce` |
| Six per-collection TIR store allocation and retrieval tests consolidated in Phase 14C15a | Templates/nodes allocate independent sequential IDs and all four side vectors return their exact typed payloads | `store_tests::{push_returns_sequential_ids_per_collection,typed_retrieval_returns_stored_entry}` | out-of-bounds safety and four wrapper-set structural reuse/non-reuse owners remain separate | `8e89329ce` |
| `summary_tests::{default_summary_is_empty,summary_fields_are_mutable}` | Default/empty summary shape and direct public-field write/read round trips | `summary_tests::{empty_summary_has_zero_counts_and_false_flags,record_helpers_preserve_summary_shape_contracts,merge_converted_wrapper_tree_adds_counts_and_clears_const_evaluable}` | complete default construction is exercised throughout TIR builders; direct field mutability is implementation shape rather than a separate semantic invariant | `8e89329ce` |
| `builder_tests::{builder_starts_with_empty_store_view,push_text_node_returns_sequential_node_ids,finish_template_returns_sequential_template_ids,occurrence_ids_available_on_stored_nodes}` | Empty builder/store view, sequential builder pass-through IDs, and availability of stored occurrence IDs | `store_tests::{store_starts_empty,push_returns_sequential_ids_per_collection}` plus `builder_tests::{slot_occurrence_ids_assigned_in_document_order,child_template_occurrence_ids_assigned_in_document_order,expression_site_ids_assigned_in_document_order}` | 13 additional builder tests retain exact payload, metadata, encapsulation, derived-root, fresh-allocation, branch, and shared-counter facts | `e140e522a` |
| Four separate conditional, collection, and range loop-header expression-site tests consolidated in Phase 14C15b | Loop-header variants allocate their exact expression-site shapes in document order | `builder_tests::{loop_conditional_and_collection_headers_each_assign_one_expression_site,loop_range_header_assigns_start_end_and_optional_step_sites}` | `expression_sites_share_one_document_order_counter` retains the cross-node-family counter invariant | `e140e522a` |
...
| `wrapper_context_fold_tests::{prepared_fold_applies_if_child_emits_wrapper_when_child_outputs,prepared_fold_skips_if_child_emits_wrapper_when_child_has_no_output}` consolidated | `IfChildEmits` wrappers apply only when the child structurally emits output | `wrapper_context_fold_tests::prepared_fold_applies_if_child_emits_only_when_the_child_emits_output`, with exact `beforechildafter` and `NoOutput` rows | none | `149d312f7` |
...
| `fold_cache_tests::{cache_key_equality_for_identical_fields,cache_key_inequality_for_each_identity_dimension}` and the empty/active-binding plus resolved/missing-slot pairs consolidated | Exact fold-cache key identity, cache eligibility, and slot-overlay outcomes | `fold_cache_tests::{cache_key_equality_and_inequality_across_identity_dimensions,fold_view_caches_empty_binding_results_but_not_active_bindings,fold_view_slot_overlay_resolves_filled_and_missing_to_empty}` with labelled exact rows | miss/hit/overwrite, prepared identity, phase, authority, nested-context, runtime-plan, malformed-root, direct-node-cycle, and attribution owners remain separate | `e0d71bb7f` |
